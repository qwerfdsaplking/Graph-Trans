...
class PYGDatasetLookupTable:
    ...
    def GetPYGDataset(dataset_spec: str, seed: int) -> Optional[Dataset]:
        ...
        if name == "qm7b":
            ...
            train_set = MyZINC(root=root,subset=True, split="train")
            valid_set = MyZINC(root=root,subset=True, split="val")
            test_set = MyZINC(root=root,subset=True, split="test")
